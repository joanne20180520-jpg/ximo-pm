...
function normalizeApprovalInstanceStatus(detail) {
  const raw = (detail && (
    detail.status || detail.instance_status || detail.approval_status || detail.instanceStatus
  )) || '';
  return String(raw).trim().toUpperCase();
}

function isApprovalInstanceApprovedStatus(st) {
  const s = String(st || '').trim().toUpperCase();
  return s === 'APPROVED' || s === 'DONE' || s === 'COMPLETED' || s === 'SUCCESS' || s === 'PASSED';
}

function isApprovalDetailEffectivelyApproved(detail) {
  const st = normalizeApprovalInstanceStatus(detail);
  if (isApprovalInstanceApprovedStatus(st)) return true;
  ...
  tasks.forEach(function(task) {
    const ts = String((task && task.status) || '').toUpperCase();
    if (ts === 'PENDING' || ts === 'IN_PROGRESS' || ts === 'PROCESSING') pending++;
    else if (ts === 'APPROVED' || ts === 'DONE' || ts === 'PASSED') approved++;
  });
  // 所有簽核節點都已通過（可能還有 CC），視為可核銷
...
function paymentMatchesApprovalDetail(fields, detail, widgets) {
  ...
  const formAmount = extractApprovalFormAmount(formValues, detail) || (
    paymentMatchesApprovalForm(fields, formValues)
      ? amount
      : 0
  );
  if (!amount || !formAmount || Math.abs(formAmount - amount) > 0.009) {
    if (!paymentMatchesApprovalForm(fields, formValues)) return false;
  }
  const payee = String(fields['支付對象'] || '').trim();
  const reason = String(fields['事由'] || '').trim();
  const formPayee = String(formValues['支付對象'] || formValues['收款人'] || '').trim();
  const formReason = String(formValues['事由'] || formValues['付款事由'] || '').trim();
  // 事由／對象過短（如測試用「s」）時，必須再對日期，避免多筆互搶同一審批單
  const weakText = (payee.length <= 2 && reason.length <= 2);
  const appDate = parsePaymentTs(fields['申請日期']);
  const startMs = paymentDetailStartMs(detail);
  if (weakText) {
    if (!appDate || !startMs) return false;
    return Math.abs(appDate.getTime() - startMs) < 2 * 86400000;
  }
  if (paymentMatchesApprovalForm(fields, formValues)) return true;
  if (payee && formPayee && (payee === formPayee || formPayee.indexOf(payee) >= 0 || payee.indexOf(formPayee) >= 0)) {
    if (appDate && startMs && Math.abs(appDate.getTime() - startMs) > 7 * 86400000) return false;
    return true;
  }
  if (reason && formReason && (reason === formReason || formReason.indexOf(reason) >= 0 || reason.indexOf(formReason) >= 0)) {
    if (appDate && startMs && Math.abs(appDate.getTime() - startMs) > 7 * 86400000) return false;
    return true;
  }
  if (appDate && startMs && Math.abs(appDate.getTime() - startMs) < 3 * 86400000 && Math.abs(formAmount - amount) <= 0.009) {
    return true;
  }
  return false;
}

async function resolvePaymentInstanceCode(token, paymentRec, widgets, detailCache, usedCodes) {
...
function normalizeAccCompany(raw) {
  const t = String(raw || '').trim();
  if (!t) return '';
  if (t === '身土' || t.indexOf('身土') >= 0 || t.indexOf('身士') >= 0) return '身土';
  if (t === '璽墨' || t.indexOf('璽墨') >= 0 || t.indexOf('喜墨') >= 0) return '璽墨';
  return t;
}
...
async function loadPaymentCompanyMap(ximoToken) {
  const map = {};
  try {
    const frontCfg = paymentsFrontConfig();
    const tableId = await resolvePaymentsTableId(ximoToken, frontCfg.appToken, frontCfg.tableId);
    if (!tableId || !frontCfg.appToken) return map;
    const rows = await getRecords(ximoToken, tableId, frontCfg.appToken);
    rows.forEach(function(rec) {
      if (!rec || !rec.record_id) return;
      const company = paymentCompanyValue(rec.fields || {});
      if (company) map[rec.record_id] = company;
    });
  } catch (e) {
    console.warn('loadPaymentCompanyMap', e.message || e);
  }
  return map;
}
 ...
 * 4) 同一來源支出ID 多列時只留一筆
 */
async function backfillAccCompanyFromPayments(ximoToken, opts) {
  opts = opts || {};
  const dryRun = !!opts.dryRun;
  const limit = Math.max(0, parseInt(opts.limit, 10) || 0);
  const out = {
  ...
  for (let i = 0; i < accExpenses.length; i++) {
    ...
    try {
      if (!dryRun) {
        await updateRecord(accToken, ACC_TABLE_EXPENSES, rec.record_id, { '公司': company }, ACC_APP_TOKEN, false);
      }
      out.expenseUpdated++;
    } catch (e) {
      out.errors.push({ id: rec.record_id, error: e.message || String(e) });
...
function parseXimoContractAmount(fields) {
  const f = fields || {};
  const raw = f['合約金額'] != null ? f['合約金額'] : f['含稅金額'];
  const n = Number(raw);
  return !isNaN(n) && n > 0 ? n : 0;
}

function parseXimoContractUntaxed(fields) {
  const f = fields || {};
  const direct = Number(f['合約未稅金額'] != null ? f['合約未稅金額'] : f['合約未稅']);
  if (!isNaN(direct) && direct > 0) return direct;
  const taxed = parseXimoContractAmount(f);
  return taxed > 0 ? Math.round(taxed / 1.05) : 0;
}
...
async function upsertAccProjectFromXimo(accToken, ximoProject, cachedAccProjects) {
  ...
  const contractTaxed = parseXimoContractAmount(fields);
  const contractUntaxed = parseXimoContractUntaxed(fields);
  const body = {
    '案名': name,
    '完整名稱': name,
...
async function syncXimoCatalogToAcc(ximoToken, opts) {
  ...
  // 增量也要撈任務／設計，否則工項執行進度算不出來，ACC 履約會卡在 0%
  let tasks = [];
  let designs = [];
  ...
  if (incremental) {
    ...
    workitems.forEach(function(wi) {
      if (wi && wi.record_id) wiIdSet[wi.record_id] = 1;
    });
    tasks = tasks.filter(function(t) {
      const ids = getLinkIds((t.fields || {})['所屬工作項目'] || (t.fields || {})['工作項目']);
      for (let i = 0; i < ids.length; i++) {
        if (wiIdSet[ids[i]]) return true;
      }
      return false;
    });
    designs = designs.filter(function(d) {
      const ids = getLinkIds((d.fields || {})['所屬工作項目'] || (d.fields || {})['工作項目']);
      for (let i = 0; i < ids.length; i++) {
        if (wiIdSet[ids[i]]) return true;
      }
      return false;
...
async function syncSettledPaymentToAccPortal(ximoToken, paymentRec) {
  ...
  if (accProject && company) {
    try { await stampAccProjectCompany(accToken, accProject, company); } catch (e) {}
  }
    ...
    if (soft) {
      if (!dryRun) {
        try {
          ...
          const softCompany = normalizeAccCompany(
            accFieldText(fields['公司']) || (paymentId ? paymentCompanyMap[paymentId] : '')
          );
          if (softCompany && !normalizeAccCompany(accFieldText((soft.fields || {})['公司']))) {
            patch['公司'] = softCompany;
          }
...
async function finalizeApprovedPaymentRecord(tenantToken, paymentRec, expenseCache, opts) {
  ...
  try {
    ...
    expenseError = e.message || String(e);
  }

  // 即便支出寫入失敗，也要先把狀態改成已核銷，避免畫面卡在審批中
...
async function matchPendingPaymentsToApprovals(tenantToken, pending) {
  ...
  const startMs = minTs - 2 * 86400000;
  const endMs = Math.max(maxTs + 3 * 86400000, Date.now());

  let instanceCodes = [];
  try {
    instanceCodes = await listApprovalInstanceCodes(tenantToken, approvalCode, startMs, endMs);
  } catch (e) {
...
function getMemberRole(fields) {
  // 「免日報」不影響登入權限；管理員全頁；PM 限時程／履約／費用／支出／付款／設計／日誌；設計主管／設計師限縮設計頁；會計可看費用管理與支出明細
...
async function fetchTableRecordsSafe(token, tableKey) {
  ...
  try {
    ...
    // 人員表必須帶 open_id，否則前端無法用 openId 對上管理員，會被誤判成 PM
    const readOpts = tableKey === 'members' ? { userIdType: 'open_id' } : undefined;
    const records = await getRecords(token, tableId, tableAppToken, readOpts);
    return { records: records };
  } catch (err) {
...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (action === 'audit-stuck-payment-approvals' && (req.method === 'GET' || req.method === 'POST')) {
      try {
        const token = await getToken();
    ...
    if (action === 'backfill-acc-company' && (req.method === 'GET' || req.method === 'POST')) {
      try {
        const token = await getToken();
        const q = req.query || {};
        const b = req.body || {};
        const dryRun = String(q.dryRun || b.dryRun || '') === '1'
          || String(q.dryRun || b.dryRun || '').toLowerCase() === 'true';
        const limit = parseInt(q.limit || b.limit || '200', 10);
        const result = await backfillAccCompanyFromPayments(token, {
          dryRun: dryRun,
          limit: dryRun ? 0 : (isNaN(limit) ? 200 : limit)
        });
        return res.status(200).json({ ok: true, backfill: result });
      } catch (err) {
        return res.status(500).json({ ok: false, error: err.message || String(err) });
      }
    }
    ...
    if (req.method === 'PUT') {
      ...
      // 標案／工項變更後同步到 ACC（合約金額、執行進度）
      if (table === 'projects' || table === 'workitems') {
        try {
          let onlyIds = [];
          if (table === 'projects') {
            onlyIds = [recordId];
          } else {
            onlyIds = getLinkIds(cleanBody['所屬標案'] || cleanBody['所數標案']);
            if (!onlyIds.length) {
              try {
                const existingWi = await getRecordById(tenantToken, tid, recordId, tableAppToken);
                onlyIds = getLinkIds(
                  (existingWi && existingWi.fields && (existingWi.fields['所屬標案'] || existingWi.fields['所數標案'])) || []
                );
              } catch (e) {}
            }
          }
          result.accSync = await syncXimoCatalogToAcc(tenantToken, {
            onlyProjectIds: onlyIds.length ? onlyIds : null,
            skipPrune: true,
            skipExpenseRematch: true
          });
        } catch (accErr) {
